...
class OrderService
{
    ...
    public function history(int $userId) { return $this->orders->ofUser($userId); }
    public function list() { return $this->orders->all(); }
    // Chi tiết 1 đơn (kèm items + product) cho trang xem đơn
    public function find(int $id) { return $this->orders->find($id); }
    public function updateStatus(int $id, string $status) { $this->orders->updateStatus($id, $status); }
}
